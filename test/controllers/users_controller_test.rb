...
class UsersControllerTest < ActionController::TestCase
  ...
  test 'should activate user' do
    assert_difference('User.current.count') do
      post :activate, params: { user: { first_name: 'First Name', last_name: 'Last Name', email: 'activated@example.com', status: 'active', pp_committee: true, pp_committee_secretary: true, steering_committee: true, steering_committee_secretary: true, system_admin: true } }
    end
  ...
  test 'should not activate user with blank name' do
    assert_difference('User.current.count', 0) do
      post :activate, params: { user: { first_name: '', last_name: '', email: '', status: 'active' } }
    end
  ...
  test 'should update user' do
    put :update, params: { id: @user, user: { first_name: 'FirstName', last_name: 'LastName', email: 'valid_updated_email@example.com', status: 'active', system_admin: false } }
    assert_redirected_to user_path(assigns(:user))
  end

  test 'should update user and set user active' do
    put :update, params: { id: users(:pending), user: { status: 'active', first_name: users(:pending).first_name, last_name: users(:pending).last_name, email: users(:pending).email, system_admin: false } }
    assert_equal 'active', assigns(:user).status
    assert_redirected_to user_path(assigns(:user))
  end

  test 'should update user and set user inactive' do
    put :update, params: { id: users(:pending), user: { status: 'inactive', first_name: users(:pending).first_name, last_name: users(:pending).last_name, email: users(:pending).email, system_admin: false } }
  ...
  test 'should not update user with invalid id' do
    put :update, params: { id: -1, user: { first_name: 'FirstName', last_name: 'LastName', email: 'valid_updated_email@example.com', status: 'active', system_admin: false } }
